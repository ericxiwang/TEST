...
def event_gen(current_date,x,http_post_enable):
	...
	for daily_birth in range(amount_birth):
		...
		def random_time(datetime_input):
			...
			date_with_random = datetime.datetime.utcfromtimestamp(datetime_input) + datetime.timedelta(hours=random.randint(0,14))
	...
	######################### Retention as follow #####################	
		if len(daily_user_list) == amount_birth:	# when all distinct_id are pushed into list, do as follow(retention start)

			print "--------start retetion----------"

			fate_percentage = [0.55,0.49,0.43,0.37,0.31,0.25,0.2,0.12,0.08] 	######## array of retention rate ########
			...
			for fade_rate in range(1,9):
				...
				datetime_offset_format = time.strptime(str(datetime_offset), "%Y-%m-%d %H:%M:%S")	# Transfer to date time format

				datetime_offset_1 = int(time.mktime(datetime_offset_format))		# Transfer to ctime format, put int function
				...
				sample_amount = int(len(daily_user_list)*real_percentage)	# define how many retention users in advance(percentage)
				...
				for daily_retention in range(sample_amount):
					...
					YA0start_retention = package_gen_duration.package_generator("YA0start","2.0.2","8416e32af87f11e284c212313b0ace15",random_time(datetime_offset_1),daily_user_list[daily_retention])
